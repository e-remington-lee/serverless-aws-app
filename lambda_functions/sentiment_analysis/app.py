...
def lambda_handler(event, context):
    bucket_name_str = g_bucket_name_str
...
def comprehend_dis(start_int, end_int, length_int, safe_list):
    ...
    sentiment = _comp.batch_detect_sentiment(TextList=my_list,LanguageCode='en')
    for j_int in range(len(sentiment["ResultList"])):
        # print(start_int, end_int)

        every_item = {}
        every_item["Sentiment"] = sentiment["ResultList"][j_int]["Sentiment"]
        every_item["SentimentScore"] = sentiment["ResultList"][j_int]["SentimentScore"]
